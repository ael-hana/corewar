/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   op.c                                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: zaz <marvin@42.fr>                         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2013/10/04 11:43:01 by zaz               #+#    #+#             */
/*   Updated: 2016/03/18 04:29:38 by ael-hana         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "op.h"

t_op    op_tab[17] =
{
	//ind = 11 dir = 10 reg = 01 
	{"live", 1, {T_DIR}, 1, 10, "alive", 0, 0},// done
	{"ld", 2, {T_DIR | T_IND, T_REG}, 2, 5, "load", 1, 0},//done
	{"st", 2, {T_REG, T_IND | T_REG}, 3, 5, "store", 1, 0},//done
	{"add", 3, {T_REG, T_REG, T_REG}, 4, 10, "addition", 1, 0},//done
	{"sub", 3, {T_REG, T_REG, T_REG}, 5, 10, "soustraction", 1, 0},//done
	{"and", 3, {T_REG | T_DIR | T_IND, T_REG | T_IND | T_DIR, T_REG}, 6, 6,
		"et (and  r1, r2, r3   r1&r2 -> r3", 1, 0},							//done
	{"or", 3, {T_REG | T_IND | T_DIR, T_REG | T_IND | T_DIR, T_REG}, 7, 6,
		"ou  (or   r1, r2, r3   r1 | r2 -> r3", 1, 0},						//done
	{"xor", 3, {T_REG | T_IND | T_DIR, T_REG | T_IND | T_DIR, T_REG}, 8, 6,
		"ou (xor  r1, r2, r3   r1^r2 -> r3", 1, 0},					//done
	{"zjmp", 1, {T_DIR}, 9, 20, "jump if zero", 0, 1}, //carry 1 done
	{"ldi", 3, {T_REG | T_DIR | T_IND, T_DIR | T_REG, T_REG}, 10, 25,
		"load index", 1, 1},											//done
	{"sti", 3, {T_REG, T_REG | T_DIR | T_IND, T_DIR | T_REG}, 11, 25,
		"store index", 1, 1},											//done
	{"fork", 1, {T_DIR}, 12, 800, "fork", 0, 1},						//done
	{"lld", 2, {T_DIR | T_IND, T_REG}, 13, 10, "long load", 1, 0},		//done
	{"lldi", 3, {T_REG | T_DIR | T_IND, T_DIR | T_REG, T_REG}, 14, 50,
		"long load index", 1, 1},										//done
	{"lfork", 1, {T_DIR}, 15, 1000, "long fork", 0, 1},					//done
	{"aff", 1, {T_REG}, 16, 2, "aff", 1, 0},							//done
	{0, 0, {0}, 0, 0, 0, 0, 0}
};
