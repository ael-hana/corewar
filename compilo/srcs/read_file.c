...
/*   Updated: 2016/03/10 20:15:19 by tle-meur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	*size += len;
		...
		&& ((bufs[0] = jump_whitespaces(bufs[0] + len + 1)) || 1))
	{
		if ((++(*nb)) && **bufs && !is_in_buf(**bufs, COMMENT_CHARS))
			return (0);
		ft_memcpy(bufs[2], bufs[1], len + *size);
...
static int	get_header(t_env *e, int fd, char *line, int i)
{
	...
	while ((vars[3] = ft_gnl(fd, &line)) > 0 && ++i)
	{
		*bufs = jump_whitespaces(line);
		...
		while (**bufs)
		{
			if (!ft_strcmp(*bufs, NAME_CMD_STRING) && vars[2] != 2
				&& !*(bufs[2] = e->header.prog_name))
				vars[2] = 1;
			else if (!ft_strcmp(*bufs, COMMENT_CMD_STRING) && vars[2] != 1
